...
cmd_progthread(void *ptr, unsigned long nargs)
{
	...
	int result;
	int i;
	char *const *pgm_args[nargs-1];

	assert(nargs >= 1);
	
	//Copy all of the actual program arguments into a new array
	for(i=0; i<nargs-1; i++)
	{
		pgm_args[i] = args[i+1];
	}
	...
	//result = execv(progname, pgm_args);
	if (result) {
...
common_prog(int nargs, char **args)
{
	...
	pid_t *pid = (kmalloc(sizeof(pid_t)));
  ...
  kprintf("thread with pid == %d terminated with exit status == %d\n", pid1, *status);
