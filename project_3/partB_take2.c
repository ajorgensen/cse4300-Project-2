...
/* call the subroutine (DOTPRODUCT or SAXPY) to aggregate results from each thread*/
  matvec_SAXPY(me, matrix_A, split_matrix_B, result_matrix);

  printf("Function me=%d: done with parm=%d\n", me, self);
}

int matvec_DOTPRODUCT(int thread_id, int split_A[M][N], int split_B[N][K/THREAD_COUNT], int result[M][K])
{
  int i, j, k; 
...
int matvec_SAXPY(int thread_id, int split_A[M][N], int split_B[N][K/THREAD_COUNT], int result[M][K])
{
  int i, j, k; 
  for (k=0; k<(K/THREAD_COUNT); k++)
    for (j=0; j<N; j++)
